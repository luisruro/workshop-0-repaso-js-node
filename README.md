...
```javascript
//Se llama el botón del HTML mediante el ID y se le agrega el evento de click para que cuando
//se le haga click ejecute la función fetchPost();
document.getElementById('fetch-posts').addEventListener('click', () => {
    fetchPosts();
});

//Se crea la función para traer los datos de la API
const fetchPosts = () => {
    fetch('https://jsonplaceholder.typicode.com/posts') // Llamamos los datos de la API
        .then(response => {
            if (!response.ok) {
                throw new Error('Network response was not ok ' + response.statusText);
            }
            return response.json(); // se convierten los datos de JSON a JavaScript
        })
        .then(posts => {
            displayPosts(posts);
        })
...
//Se crea la función que va desplegar los datos de la API en el HTML
const displayPosts = (posts) => {
    const postList = document.getElementById('post-list'); //Llamamos el elemento lista por el ID
    postList.innerHTML = '';
    posts.forEach(post => {
        const listItem = document.createElement('li'); //se crea el elemento que va listar los datos dentro de la lista
        listItem.textContent = `Title: ${post.title}`; // sellama el valor de titulo
        postList.appendChild(listItem); // se agrega el elemento creado con la información al HTML
    });
};
